...
if ! egrep '^[[:space:]]*CW_AUTOMACROS(\(\[[[:digit:]]*\]\)|)[[:space:]]*([[:space:]]dnl[[:space:]]|$)' configure.ac > /dev/null; then
  echo -e "\nERROR: configure.ac must contain CW_AUTOMACROS([$(cat @INSTALLPREFIX@/share/cwautomacros/version)]). Put it for example at the top below AM_MAINTAINER_MODE."
  exit 1
fi
...
if ! grep '^[[:space:]]*ACLOCAL_AMFLAGS[[:space:]]*=' Makefile.am >/dev/null; then
  echo -e "\nERROR: ACLOCAL_AMFLAGS not set in Makefile.am, it should contain \"-I @INSTALLPREFIX@/share/cwautomacros/m4\". Make sure you set it in all Makefile.am's."
  exit 1
fi
